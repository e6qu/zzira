# Work item comments, links, archiving and bulk operations

Work item REST resources: comments and their properties, links and link types, remote links, watchers and assignment, reading and writing work items, changelogs, the issue picker, notifications, limit reports, archiving, redaction, bulk properties and issue panel pins. Part of the [Jira platform](JIRA_PLATFORM.md). For status, see [CLOUD_PARITY.md](CLOUD_PARITY.md). Related: [attachments](ATTACHMENTS.md), [worklogs](WORKLOGS.md), [bulk operations](BULK_ISSUES.md), [fields](ISSUE_FIELDS.md).

## Ids

Work items, comments, links, link types, remote links, attachments and worklogs use Jira's numeric ids on the wire, including in `self` URLs (for example `/rest/api/3/issue/10042/comment/10007`). Lookups accept those ids, and keys for work items. Internal ids are never sent. See [WIRE_IDS.md](WIRE_IDS.md).
...
people outside it do not see the comment in any list, lookup or work item field.
...
all comments* or *Delete own comments* to delete. A work item holds at most 5000
...
A link reads from the inward work item: with Blocks, `inwardIssue` "blocks"
`outwardIssue`. The inward work item's `issuelinks` field lists the other work item as
`outwardIssue`; the outward work item lists it as `inwardIssue`. Creating a link
needs *Link issues* on the inward work item's project; an optional `comment` goes on
the outward work item. Each work item holds at most 2000 links. Links are refused with
...
already on the work item replaces that link (200); otherwise it creates one (201).
Fields left out of `POST` or `PUT` become empty. A link id that belongs to another
work item is 400; an unknown one is 404. Changes need *Link issues* and *Edit issues*.
...
[notification schemes](NOTIFICATION_SCHEMES.md)).
...
person must be able to see the work item. `DELETE` requires `accountId`.
...
`versionedRepresentations`), `properties` and `updateHistory`. Beyond the work item
...
- `PUT /rest/api/3/issue/{issueIdOrKey}` stores `properties` and answers 200 with
  ...
  transitions, its edit metadata lists no fields, and the work item page hides Edit
...
- `POST /rest/api/3/changelog/bulkfetch` returns the changelogs of up to 1000
  work items, oldest first and then by work item id, optionally only items for up to 10
...
matching work items the caller viewed most recently, and *Current Search* (`cs`), the
matching work items of `currentJQL`. `query` matches a key or summary and is wrapped
...
current work item's parent. Archived work items never appear.
...
and only to people who can see the work item. The sender is never a recipient;
...
- `GET /rest/api/3/issue/limit/report` reports work items at or above 80% of the
  per-item limits: comments 5000, worklogs 10000, attachments 2000, links
...
`isReturningKeys=true` keys both reports by work item key instead of id.
...
Archiving and restoring take a work item's subtasks with it; a subtask cannot be
named directly. The answer counts the work items changed and groups the rest under
`issuesNotFound`, `issueIsSubtask`, `issuesInArchivedProjects`,
`issuesInUnlicensedProjects` and `userDoesNotHavePermission`: 200 when any work item
changed, 400 when none did, 412 when more than 1000 were named.

An archived work item can still be read, but it leaves search, boards and the picker,
...
to a CSV of the matching archived work items.

## Redaction

`POST /rest/api/3/redact` queues redactions of text in work item fields (`summary`,
...
## Bulk work item properties

All four operations answer 303 with a task:

- `POST /rest/api/3/issue/properties` sets up to 10 properties on up to 10000 work items.
- `POST /rest/api/3/issue/properties/multi` sets per-item properties on up to 100 work items.
- `PUT /rest/api/3/issue/properties/{key}` sets one value on work items chosen by
  `filter` (`entityIds`, `currentValue`, `hasProperty`).
- `DELETE /rest/api/3/issue/properties/{key}` removes a property from work items chosen
...
`POST /rest/api/3/forge/panel/action/bulk/async` pins or unpins an installed work item
