...
| Admin automates work | Partial | ✅ Fixed-interval and Quartz cron scheduled triggers, work item created, transitioned, field changed and commented triggers with JQL scope, work item fields conditions, label/assign/transition/comment/edit summary and due date actions with smart values, rule-to-rule trigger control, management, run-now and audit; branches, the rest of the trigger/condition/action catalog and a template gallery in the browser remain |
