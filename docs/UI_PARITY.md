...
| Service customer requests help | Partial | ✅ Help-center and portal discovery, linked knowledge search/article viewing, request-type-specific forms with required text, number, date-time, date, URL, label, select, multi-select, cascading select, email-matched user picker, and group, project and version picker fields shown conditionally on choice answers, typed help/incident/problem/change request submission, owned request list/detail with structured answers, participant add/remove, public conversation and files, approvals, subscription controls, completed-request feedback, customer-visible SLA state, organization visibility, and status transitions with light/dark accessibility and 320 px reflow; team and Assets-backed form fields remain |
