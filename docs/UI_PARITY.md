...
| Contributor follows code through release | 🟡 | `directories` (development information), `software`, `releases`, `plans_setup` (cross-project releases), `releases` (the hub reading a cross-project release back, the environments the work has been through, and the project's release conditions) | Promotion policies; promotion itself is the delivery provider's ([RELEASES](RELEASES.md#environments)) |
...
| Manager plans across teams | 🟡 | `timeline`, `plans_view`, `plans_teams`, `plans_setup` (create, scheduling, sources, exclusions, cross-project releases, access) | Auto-scheduler ([JIRA_SOFTWARE](JIRA_SOFTWARE.md#gaps)) |
...
| Engineering manager reviews delivery | ✅ | `releases` (DORA metrics), `dora_mapping` (the environments, pipelines, incidents and excluded periods that count), `reports_delivery` (deployment frequency, cycle time), `dashboard_reports` (the delivery gadgets), `report_subscriptions` | — |
...
| Admin shapes the work item model | 🟡 | `issue_metadata` (work types and their schemes, priorities, resolutions), `custom_field_contexts`, `custom_field_options`, `field_translations`, `issue_metadata_translations` (work types, priorities and resolutions named per language), `field_configurations`, `screens`, `screen_schemes` | A translated name reaches the pages; JQL reads the site's own words, as Jira's does ([ISSUE_METADATA](ISSUE_METADATA.md#gaps)) |
| Admin structures work above the epic | 🟡 | `hierarchy` (add a level, move a work type onto it, parent an epic under it, roll the roadmap up through it, and report on it) | Boards and backlogs are epic-level, as Jira's are ([ISSUE_METADATA](ISSUE_METADATA.md#work-type-hierarchy)) |
...
| Admin automates work | 🟡 | `automation` (schedules, work item events including assigned, attachment added, deleted, moved, versions and sprints, wiki pages written, changed, commented and labelled, webhooks, manual rules run from a work item or over a selection, with the questions they ask, branches over related work, a query, the work the rule created or each item in a list, rule variables, looking work items up, conditions, the action catalog) | Branches inside branches; Confluence actions beyond writing, commenting on, labelling, appending to and archiving a page; the rest of the catalog ([AUTOMATION](AUTOMATION.md#gaps)) |
| Service customer requests help | 🟡 | `service`, `service_assets` | AQL references and functions ([SERVICE_MANAGEMENT](SERVICE_MANAGEMENT.md#assets-filters)) |
| Service agent works a queue | 🟡 | `service`, `service_assets` | A queue is written in the site's own JQL, including JSM's `Organizations`; a request is shared with an organization by who raised it rather than one at a time ([JQL](JQL.md#gaps)) |
| Service manager runs a service | ✅ | `service`, `service_assets` (schemas, objects, topology, impact, import, comments, history and files) | — |
| Admin starts a service project | ✅ | `service` (creation, and archiving, restoring, trashing and deleting one with its portal) | — |
...
| Space manager governs knowledge | 🟡 | `wiki_space_admin` (details, custom roles, direct grants), `wiki_space_tools` (export and import a space), `wiki_page_lifecycle`, `classification_levels` | An imported space carries its pages, blog posts, labels, comments and files, not restrictions or page history ([CONFLUENCE_SPACES](CONFLUENCE_SPACES.md#gaps)) |
| Site admin manages people, access and Jira settings | 🟡 | `admin`, `password`, `two_step`, `api_tokens`, `people`, `global_permissions`, `identity-providers`, `permission_schemes`, `permission_helper`, `notification_schemes`, `notification_helper`, `notification_preferences`, `notifications`, `issue_security_schemes` | SAML; two-step verification is an authenticator app and no other second factor; a SCIM provider is pointed at the address `/admin` shows and holds a key issued there ([ADMIN](ADMIN.md), [SCIM](SCIM.md)) |
