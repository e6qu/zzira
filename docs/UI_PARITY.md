...
| Manager plans across teams | 🟡 | `timeline`, `plans_view`, `plans_teams`, `plans_setup` (create, sources, exclusions, access) | Auto-scheduler; scheduling fields and estimation unit in the browser; rollups above epic; releases in plans; velocity-based capacity; plan views ([JIRA_SOFTWARE](JIRA_SOFTWARE.md#gaps)) |
...
| Engineering manager reviews delivery | 🟡 | `releases` (DORA metrics), `dora_mapping` (choosing the environments and pipelines that count), `report_subscriptions` | Which work items count as incidents; excluded-period calendars ([REPORTS](REPORTS.md#gaps)) |
