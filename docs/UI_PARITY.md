...
| Admin automates work | Partial | ✅ Fixed-interval and Quartz cron scheduled triggers, work item created, transitioned, field changed and commented triggers with JQL scope, work item fields conditions, label/assign/transition/comment/edit summary and due date actions with smart values, sub-task, parent and linked work item branches through the rule API, rule-to-rule trigger control, management, run-now and audit; editing branches and JQL conditions in the browser, the rest of the trigger/condition/action catalog and a template gallery in the browser remain |
