...
| Admin designs and publishes a workflow | Partial | ✅ Directory, diagram, custom transition drafts, published-version isolation, explicit publish/discard and project assignment, plus API-configured nested conditions, required-field validators and assignee post-functions enforced in browser transitions; visual rule editing, status editing and complete scheme administration remain |
