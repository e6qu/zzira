...
| Service customer requests help | Partial | ✅ Help-center and portal discovery, typed help/incident request submission, owned request list/detail, participant add/remove, public conversation, status and condition-aware transitions with light/dark accessibility and 320 px reflow; knowledge suggestions, dynamic forms, attachments, approvals, notifications and feedback remain |
| Service agent works a queue | Partial | ✅ Per-desk agent assignment/revocation, assigned-desk navigation, scoped all-request reads, all-open/unassigned/assigned-to-me views, responsive request table, take/unassign ownership, raise-on-behalf, request detail, participants, private collaboration and workflow actions; custom queues, SLAs, approvals and escalation remain |
| Service manager runs a service | Partial | ✅ Site administrators manage each desk's agent roster with immediate permission changes; portal/request-type setup, forms, calendars, SLAs, Assets, incident/problem/change configuration and reporting remain |
