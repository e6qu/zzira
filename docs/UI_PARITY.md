...
| Service customer requests help | Partial | ✅ Help-center and portal discovery, linked knowledge search/article viewing, request-type-specific forms with required text, number, date-time, date, URL, label, select, multi-select, cascading select and email-matched user picker fields, typed help/incident/problem/change request submission, owned request list/detail with structured answers, participant add/remove, public conversation and files, approvals, subscription controls, completed-request feedback, customer-visible SLA state, organization visibility, and status transitions with light/dark accessibility and 320 px reflow; conditional, group, project, version, team and Assets-backed form fields remain |
