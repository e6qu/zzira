...
| Engineering manager reviews delivery | 🟡 | `releases` (DORA metrics), `dora_mapping` (choosing the environments and pipelines that count), `reports_delivery` (deployment frequency, cycle time), `report_subscriptions` | Which work items count as incidents; excluded-period calendars ([REPORTS](REPORTS.md#gaps)) |
