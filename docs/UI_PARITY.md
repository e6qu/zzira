...
| Contributor follows code through release | 🟡 | `directories` (development information), `software`, `releases`, `plans_setup` (cross-project releases, and the release hub reading one back), `releases` (release conditions) | Promotion policies; promotion itself is the delivery provider's ([RELEASES](RELEASES.md#environments)) |
