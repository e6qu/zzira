...
| Admin automates work | 🟡 | `automation` (schedules, work item events including assigned, attachment added, deleted, moved, versions and sprints, wiki pages written, changed, commented and labelled, webhooks, manual rules run from a work item with the questions they ask, branches over related work, a query, the work the rule created or each item in a list, rule variables, looking work items up, conditions, the action catalog) | Running a manual rule over a selection; branches inside branches; Confluence actions beyond writing, commenting on and labelling a page; the rest of the catalog ([AUTOMATION](AUTOMATION.md#gaps)) |
