...
| Service customer requests help | Partial | ✅ Help-center and portal discovery, typed help/incident request submission, owned request list/detail, participant add/remove, public conversation, customer-visible SLA state, status and condition-aware transitions with light/dark accessibility and 320 px reflow; knowledge suggestions, dynamic forms, attachments, approvals, notifications and feedback remain |
| Service agent works a queue | Partial | ✅ Per-desk agent assignment/revocation, assigned-desk navigation, scoped all-request reads, all-open/SLA-attention/unassigned/assigned-to-me views, responsive request table, take/unassign ownership, raise-on-behalf, request detail, participants, private collaboration, workflow actions, SLA cycles and deduplicated warning/breach notifications; custom queues and approvals remain |
| Service manager runs a service | Partial | ✅ Site administrators manage each desk's agent roster, business calendar and first-response/resolution goals with immediate permission/configuration changes; portal/request-type setup, conditional goals, holiday management, Assets, incident/problem/change configuration and reporting remain |
| Admin starts a service project | Partial | ✅ Service-management template creation, service-project identity, atomic desk provisioning, default help and incident request types, request field metadata, customer creation, agent roster, all-request visibility and internal notes; portal configuration and the full service lifecycle remain |
