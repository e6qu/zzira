...
| Admin designs and publishes a workflow | Partial | ✅ Global or project-scoped creation, automatic project assignment, connected status map, drag and keyboard layout editing, serialized draft saves, custom transition drafts with visual reporter/assignee restrictions, required fields, assignee effects and screen fields, published-version isolation, explicit publish/discard, plus nested API rule configuration enforced in browser transitions; advanced rule parameters and complete scheme administration remain |
