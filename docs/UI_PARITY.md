...
| Service manager runs a service | Partial | ✅ Site administrators manage each desk's request-type field forms/help/requirements, custom JQL queues, agent roster, open/closed customer admission, customer invitation/membership, linked Confluence spaces, business calendar and first-response/resolution goals; agents manage customer organizations, membership, properties and desk links; conditional fields/goals, holidays, complete Assets administration, incident/problem/change configuration and reporting remain |
