...
| Knowledge user authors and discusses a page | Partial | ✅ Space/page create, permission-filtered page and nested-folder trees, folder creation beneath pages or folders and empty-folder deletion, storage editor, drafts, history, cross-content search, trash/restore, labels, threaded page/attachment footer comments, exact-passage inline discussions with replies/resolution, page tasks with assignees/due dates/completion, likes/history, direct-user/group view/edit restrictions, permission-scoped attachment upload/replacement/download/deletion/labels/versioned JSON properties, and page/space/label watches that open deduplicated in-app notifications; complete editor, databases, Smart Links, whiteboards, blogs, heterogeneous child trees, manual ordering, mentions, storage-macro task extraction, rich anchor relocation, watch email delivery and live collaboration remain |
