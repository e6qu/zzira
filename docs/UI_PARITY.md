...
| Engineering manager reviews delivery | 🟡 | `releases` (DORA metrics), `dora_mapping` (choosing the environments, pipelines and excluded periods that count), `reports_delivery` (deployment frequency, cycle time), `report_subscriptions` | Which work items count as incidents ([REPORTS](REPORTS.md#gaps)) |
