...
| Admin automates work | 🟡 | `automation` (schedules, work item events including assigned, attachment added, deleted, moved, versions and sprints, wiki pages written, changed, commented and labelled, webhooks, manual rules run from a work item or over a selection, with the questions they ask, branches over related work, a query, the work the rule created or each item in a list, rule variables, looking work items up, conditions, the action catalog) | Confluence actions beyond writing, commenting on, labelling, appending to and archiving a page; the rest of the catalog, which now includes watchers and cloning ([AUTOMATION](AUTOMATION.md#gaps)) |
...
| Knowledge user authors and discusses a page | 🟡 | `wiki`, `wiki_content_tree`, `wiki_drafts_purge`, `wiki_editor`, `wiki_page_details`, `wiki_page_lifecycle`, `wiki_mentions`, `wiki_watches` | The editor writes headings, quotes, code, links, lists and tables; macros and layouts are still written as storage ([PAGE_WRITING](PAGE_WRITING.md)) |
