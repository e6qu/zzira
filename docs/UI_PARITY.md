...
| Service customer requests help | Partial | ✅ Help-center and portal discovery, typed help/incident request submission, owned request list/detail, participant add/remove, public conversation and files, approvals, subscription controls, completed-request feedback, customer-visible SLA state, organization visibility, and status transitions with light/dark accessibility and 320 px reflow; knowledge suggestions and dynamic forms remain |
...
| Service manager runs a service | Partial | ✅ Site administrators manage each desk's agent roster, open/closed customer admission, customer invitation/membership, business calendar and first-response/resolution goals; agents manage customer organizations, membership, properties and desk links; request-type setup, conditional goals, holidays, Assets, incident/problem/change configuration and reporting remain |
