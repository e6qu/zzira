...
| Admin automates work | 🟡 | `automation` (schedules, work item events including assigned, attachment added, deleted, moved, versions and sprints, webhooks, manual rules run from a work item with the questions they ask, branches, conditions, the action catalog) | Triggers for Confluence content; running a manual rule over a selection; the rest of the catalog ([AUTOMATION](AUTOMATION.md#gaps)) |
