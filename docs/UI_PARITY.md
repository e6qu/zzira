...
| Site admin manages apps | Partial | ✅ Encrypted descriptor installation and reinstallation, stable non-human principals with signed scope-enforced Jira/Agile/JSM/Confluence API access, explicit scope/module/callback review, active/suspended/uninstalled lifecycle controls, organization audit, isolated storage, host-rendered global navigation, issue panels, custom-dashboard gadgets and wiki byline items, plus descriptor-declared outbound lifecycle callbacks, JQL-filtered webhooks, four schedule intervals, durable delivery recovery and recent outcome inspection; Connect JWT/QSH and full descriptor translation, Forge-hosted compute, remote iframe modules, workflow/custom-field modules and upgrade migrations remain |
