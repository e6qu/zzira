...
| Admin designs and publishes a workflow | Partial | ✅ Directory, connected status map, drag and keyboard layout editing, serialized draft saves, custom transition drafts, published-version isolation, explicit publish/discard and project assignment, plus API-configured nested conditions, required-field validators and assignee post-functions enforced in browser transitions; visual rule editing, transition screens and complete scheme administration remain |
