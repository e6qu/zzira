...
| Dashboards | Partial | CRUD, favourites, layouts, private/user/workspace sharing, native work-item gadgets and installed host-rendered app gadgets | Group/project sharing, archive/bulk edit, subscriptions, report/remote gadgets and offline data; see [DASHBOARDS.md](DASHBOARDS.md) |
...
| App runtime | Partial | Administrator-authorized install/reinstall with encrypted shared secrets, explicit scopes, HMAC-signed fresh and replay-protected lifecycle/storage/product-API requests, stable scope-enforced app principals across Jira/Agile/JSM/Confluence REST, isolated versioned JSON storage, scope-safe upgrades, suspension/uninstall, organization audit, and host-rendered Jira/Confluence global pages, issue panels, dashboard gadgets and content byline items; core webhooks and entity properties | Connect JWT/QSH and descriptor parity, outbound lifecycle delivery, remote modules, workflow/custom-field modules, app webhooks, scheduled functions and upgrade migrations; see [APPS.md](APPS.md) |
