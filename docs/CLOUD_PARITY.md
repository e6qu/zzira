...
| Work items | Partial | Issue CRUD, one-level sub-task hierarchy with Jira parent fields, rich-text subset, comments, attachments, worklogs, links, watchers, versions, fields and security | Complete ADF and metadata; epic and higher-level hierarchy, components, estimates, votes, properties, bulk operations, exact expansions and permissions |
