...
|---|---:|---|---|
| Identity and authentication | Partial | Password, sessions, API tokens, simultaneous generic OIDC, Google and tenant-scoped Microsoft OIDC, Atlassian OAuth 2.0 3LO, durable provider enable/disable, provider-bound connect state, reviewed identity linking/unlink, issuer-scoped revocation, login audit and logout | Provider registration/secret rotation and organization/site/product lifecycle |
