...
assesses 165 operations: 158 partial and 7 missing. The remaining 1,042 operations
...
| Service Management | Partial | Service-project creation, atomic desk/type/queue provisioning, customer help center and request journey, customer identities and participants, public/internal conversation boundary, status transitions, incident-to-DORA linkage, per-desk agent roles and roster administration, scoped all-request access, business calendars, first-response/resolution cycles, SLA attention and durable escalation notifications, agent queue/assignment workspace, and twenty-eight pinned operations | Custom queues/JQL, dynamic forms, organizations, advanced SLA goal criteria, holidays, approvals, attachments, customer notifications, feedback, Assets, incident/problem/change configuration, service reports and remaining REST operations; see [SERVICE_MANAGEMENT.md](SERVICE_MANAGEMENT.md) |
