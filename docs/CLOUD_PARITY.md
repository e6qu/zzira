...
assesses 75 operations: 68 partial and 7 missing. The remaining 1,132 operations
...
| Workflows | Partial | Workspace-scoped custom status create/edit/delete, built-in protection, issue/board/workflow/automation impact checks, all thirteen Jira status/status-category/usage REST operations as reviewed subsets, workspace-owned workflow directory, versioned transition drafts, explicit publish/discard with audit, scoped project assignment and published-definition runtime enforcement | Project status scopes, atomic bulk status writes, scheme-derived status usage, workflow schemes, designer persistence, conditions, validators and post-functions |
