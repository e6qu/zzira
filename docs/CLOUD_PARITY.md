...
assesses 456 operations: 449 partial and 7 missing. The remaining 751 operations
...
| Search and filters | Partial | Shared permission-filtered JQL compiler with boolean/numeric/text operators, `NOT IN`, relative date functions, immutable `WAS`/`CHANGED` history predicates, deterministic multi-field ordering, query/workspace/user-bound seven-day enhanced-search cursors, legacy/enhanced/count search, reference data, permission-filtered suggestions, Jira-shaped parse, bounded bulk match, sanitize and personal-data migration resources, navigator, and all 19 pinned filter routes with private/shared visibility, owned/favorite collections, paginated search, user/group/project/role view and edit shares, favorites, columns, owner transfer and default scope | Remaining fields/functions, exact personal-data migration, app-function precomputations, exact expansions and properties, strong-consistency reconciliation, snapshot/keyset paging and scheduled subscription delivery; see [JQL.md](JQL.md) and [FILTERS.md](FILTERS.md) |
