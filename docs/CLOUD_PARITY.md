...
| Work items | Partial | Issue CRUD, one-level sub-task hierarchy with Jira parent fields, rich-text subset, comments, attachments, worklogs, links, watchers, versions, issue-bound Advanced Forms lifecycle, fields, security and single-issue JSON property lifecycle | Complete ADF and metadata; epic and higher-level hierarchy, components, estimates, votes, bulk property operations, exact expansions and permissions |
| Search and filters | Partial | Shared permission-filtered JQL compiler with boolean/numeric/text operators, `NOT IN`, relative date functions, immutable `WAS`/`CHANGED` history predicates, deterministic multi-field ordering, query/workspace/user-bound seven-day enhanced-search cursors, legacy/enhanced/count search with field selection, key aliases, names/schema/rendered expansion and requested properties, reference data, permission-filtered suggestions, Jira-shaped parse, bounded bulk match, sanitize and personal-data migration resources, app-only tenant-scoped function-precomputation paging/search/atomic updates, navigator, and all 19 pinned filter routes with private/shared visibility, owned/favorite collections, paginated search, user/group/project/role view and edit shares, favorites, columns, owner transfer and default scope | Remaining fields/functions and app-function compiler invocation, exact personal-data migration, remaining issue expansions, strong-consistency reconciliation, snapshot/keyset paging and scheduled subscription delivery; see [JQL.md](JQL.md) and [FILTERS.md](FILTERS.md) |
