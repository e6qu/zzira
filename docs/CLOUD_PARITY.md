...
| Service Management | Partial | Service-project creation, atomic desk/type/queue provisioning, customer help center and request journey, customer identities and participants, public/internal conversation boundary, status transitions, incident-to-DORA linkage, per-desk agent roles and roster administration, scoped all-request access, agent queue/assignment workspace, and twenty-six pinned info/desk/type/customer/request/queue operations | Custom queues/JQL, dynamic forms, organizations, calendars, SLAs, approvals, attachments, notifications, feedback, Assets, incident/problem/change configuration, service reports and remaining REST operations; see [SERVICE_MANAGEMENT.md](SERVICE_MANAGEMENT.md) |
