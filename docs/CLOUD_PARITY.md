...
assesses 82 operations: 75 partial and 7 missing. The remaining 1,125 operations
...
| Workflows | Partial | Workspace-scoped custom status lifecycle, all thirteen Jira status/status-category/usage operations as reviewed subsets, workspace-owned workflows, versioned workflow and scheme drafts, issue-type mappings, runtime scheme resolution, safe project impact preview/assignment, seven core workflow-scheme REST operations and audit | Project status scopes, atomic bulk status writes, remaining workflow-scheme operations and explicit status migration mappings, designer persistence, conditions, validators and post-functions |
