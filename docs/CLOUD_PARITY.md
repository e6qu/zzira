...
| Search and filters | Partial | Shared permission-filtered JQL compiler with boolean/numeric/text operators, `NOT IN`, relative date functions, immutable `WAS`/`CHANGED` history predicates, deterministic multi-field ordering, query/workspace/user-bound seven-day enhanced-search cursors, legacy/enhanced/count search, navigator, and all 19 pinned filter routes with private/shared visibility, owned/favorite collections, paginated search, user/group/project/role view and edit shares, favorites, columns, owner transfer and default scope | Remaining fields/functions, JQL reference/parse/match/sanitize/migration/precomputation resources, exact expansions and properties, strong-consistency reconciliation, snapshot/keyset paging and scheduled subscription delivery; see [JQL.md](JQL.md) and [FILTERS.md](FILTERS.md) |
