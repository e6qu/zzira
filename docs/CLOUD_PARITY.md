...
assesses 382 operations: 375 partial and 7 missing. The remaining 825 operations
...
| Wiki and knowledge | Partial | Spaces, private spaces, permission-filtered page trees with v1/v2 child/ancestor/descendant resources, nested folders, validated HTTP/HTTPS Smart Links, creator-private database and whiteboard containers with heterogeneous page/folder/link/database/whiteboard ancestry, every documented whiteboard template key and locale, bounded descendants, operation discovery, versioned JSON properties and built-in classification state, storage subset, drafts, versions, trash/restore, threaded page/attachment footer comments, page inline discussions with exact-text anchors and resolution, page tasks with assignment, due dates and completion, labels, direct-user/group page restrictions, versioned page attachments with upload, replacement, download, bounded image thumbnails, history, JSON properties and labels, public/private blog post authoring with optimistic versions, trash/restore/purge, labels and label discovery, likes, operations, versioned JSON properties, built-in classification, guarded current/history redaction (an audited compliance exception to immutable history), registered custom-content discovery and permission-filtered versioned attachments with upload/replace/download/delete UI, plus durable page/space/label watches with synchronized deduplicated notifications across v1/v2; 168 reviewed Confluence operations and UI | Remaining Confluence v1/v2; database schemas/rows/views, whiteboard canvas objects/editing and organization-defined classification levels; blog post footer/inline comments plus other non-page content; page children beneath non-page content and manual ordering; complete ADF/storage/macros including task extraction; richer media processing; Confluence-specific space roles; rich anchor relocation; watch email delivery; CQL, exports/imports and live collaboration |
