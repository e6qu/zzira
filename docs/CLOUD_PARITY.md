...
| Work items | Partial | Issue CRUD, one-level sub-task hierarchy with Jira parent fields, rich-text subset, comments, attachments, worklogs, links, watchers, durable votes with self-service REST/browser journeys, versions, canonical multi-component create/update/search/default-assignment semantics, issue-bound Advanced Forms lifecycle, fields, security and single-issue JSON property lifecycle, plus administrator-gated common-field discovery, durable edits across ten Jira field families, durable bulk watch/unwatch, deletion with attachment cleanup retries, project/type/explicit-parent moves with old-key aliases and workflow status inference, and condition-filtered transition discovery/execution with browser progress and Jira-shaped replay-safe per-item results for up to 1,000 visible items | Complete ADF and metadata; epic and higher-level hierarchy, estimates, remaining bulk field families, bulk move classifications/mandatory-field/implicit-subtask mappings, bulk transition screen-field input, bulk properties, bulk email delivery, configurable global Bulk change permission, exact expansions and permissions; see [BULK_ISSUES.md](BULK_ISSUES.md) |
