...
assesses all 1,207 operations: all 1,207 are partial and none is missing or
...
| Projects and administration | Partial | Project create/edit across business, software and service-management types; Scrum/Kanban setup; project directory/settings; stable component lifecycle/default assignment; recent-project tracking; archive/restore/trash; synchronous and durable asynchronous deletion; automatic 60-day purge; replica coherence and attachment cleanup; numeric site-wide category lifecycle and project assignment; JSON project properties; software feature state with navigation enforcement; project sender overrides; project-type and key/name validation APIs; a durable shared project-role catalog; default and project-specific user/group actors; safe role swaps; role-backed filter sharing; all 16 permission-scheme, assignment, discovery, and evaluation operations; all nine notification-scheme operations; all 20 issue-security-scheme, level, holder, and project-association operations; all 17 screen, screen-tab, and tab-field operations; all 15 screen-scheme and work-type-screen-scheme operations; all 15 field-configuration and field-configuration-scheme operations; all 14 custom-field-context operations; all seven custom-field-option operations; all 15 project-version operations; durable grants and event recipients across Jira holder types; permission-filtered project/issue/search/sync/notification surfaces; delegated project administration; plus durable Jira site feature settings, time tracking, navigator columns, editable application properties and dismissible announcements. A hundred and fifty-one project, permission, notification, issue-security, screen, and field operations and 13 site-configuration operations now have API/browser evidence; all 47 organization operations are reviewed with tested group/user/role/activity/event/domain/policy subsets and admin journeys | The /config/fieldschemes association surface, multi-select and cascading custom fields; per-work-type bulk edit field narrowing and bulk field-rule enforcement, screen-backed workflow transition screens and a read-only view form; issue-security JQL functions, level reordering and scheme copy; global permission administration; custom notification events and preferences; action-specific permission enforcement across remaining mutations; import/export; complete templates/expansions; anonymous browse/discovery; app, guest and service role actors; custom-domain sender verification; app-provided features; complete look-and-feel application; Marketplace time providers; runtime network policy enforcement and cross-organization domain ownership; remaining Jira admin APIs and journeys; see [PROJECT_GOVERNANCE.md](PROJECT_GOVERNANCE.md), [PROJECT_LIFECYCLE.md](PROJECT_LIFECYCLE.md), [PROJECT_ROLES.md](PROJECT_ROLES.md), [PERMISSION_SCHEMES.md](PERMISSION_SCHEMES.md), [NOTIFICATION_SCHEMES.md](NOTIFICATION_SCHEMES.md), [ISSUE_SECURITY_SCHEMES.md](ISSUE_SECURITY_SCHEMES.md), [SCREENS.md](SCREENS.md), [SCREEN_SCHEMES.md](SCREEN_SCHEMES.md), [FIELD_CONFIGURATIONS.md](FIELD_CONFIGURATIONS.md), [CUSTOM_FIELD_CONTEXTS.md](CUSTOM_FIELD_CONTEXTS.md), [CUSTOM_FIELD_OPTIONS.md](CUSTOM_FIELD_OPTIONS.md), [PROJECT_VERSIONS.md](PROJECT_VERSIONS.md), and [AGILE_BOARDS.md](AGILE_BOARDS.md) |
...
| Agile planning | Partial | Boards, backlog, sprints, rank, quick filters, swimlanes, WIP and card configuration, epics, estimation, Atlassian teams and the Team field, report calculations, and plans with scenarios, in-plan editing, capacity and dependencies, and board creation and deletion both over REST and from a project's settings | Board ownership, parallel sprints and the plan auto-scheduler |
...
| Releases | Partial | Version lifecycle, fix/affects assignment, progress, notes, release/archive/delete, explicit project-wide ordering with browser reordering, drivers and approvers with decisions, related work, version-picker custom fields, and release-time movement of unfinished work | Exports, cross-project releases and full permissions; see [RELEASES.md](RELEASES.md) |
| Reports and metrics | Partial | Dashboard statistics/pie tables, release progress, immutable delivery facts, permission-filtered 7/30/90 day DORA summary, and per-desk service volume/open/resolved/SLA-breach/CSAT reports with accessible daily charts and tables, previous-period comparisons, and a CSV download and scheduled email of every report | Configurable DORA mappings, and organization and priority service segments (Jira Cloud offers no DORA targets); see [REPORTS.md](REPORTS.md) |
| Dashboards | Partial | CRUD, favourites, layouts, sharing with everyone signed in, people, groups, projects and project roles, bulk edit, native work-item gadgets, created vs. resolved, resolution time, velocity and sprint burndown report gadgets, daily or weekly dashboard emails, installed host-rendered app gadgets, and signed remote standard Connect dashboard items with configuration, refresh, conditions and Connect's JavaScript API | Offline dashboard data and Connect conditions beyond signed-in and administrator checks; Jira Cloud no longer shares dashboards publicly; see [DASHBOARDS.md](DASHBOARDS.md) |
| Automation | Partial | Eight management operations, durable execution, fixed-interval and cron schedules, five work item event triggers, JQL, field and related-work conditions, a related-work branch, smart values, eleven work item actions including work item creation and outbox-queued mail to a work item's assignee, reporter or watchers, a template gallery and audit | Webhook and incoming-request triggers, connections (stored, not executed), web request actions, page creation, usage limits and the rest of the action catalog; see [AUTOMATION.md](AUTOMATION.md) |
| Notifications and collaboration | Partial | All nine notification-scheme APIs; default and project-assigned event policies; all twelve Jira recipient types; permission and issue-security-filtered core issue-event delivery to the synchronized inbox and durable leased email outbox; watching and activity | Custom events, preferences, mentions, remaining mutation events, rich email and broader collaboration semantics; see [NOTIFICATION_SCHEMES.md](NOTIFICATION_SCHEMES.md) |
