...
| Work items | Partial | Issue CRUD, one-level sub-task hierarchy with Jira parent fields, rich-text subset, comments, attachments, worklogs, links, watchers, durable votes with self-service REST/browser journeys, versions, canonical multi-component create/update/search/default-assignment semantics, issue-bound Advanced Forms lifecycle, fields, security and single-issue JSON property lifecycle, plus administrator-gated common-field discovery, durable edits across ten Jira field families, durable bulk watch/unwatch, and durable bulk deletion for up to 1,000 visible items with execution-time access checks, replay safety, attachment cleanup retries, browser progress and Jira-shaped per-item results | Complete ADF and metadata; epic and higher-level hierarchy, estimates, remaining bulk field families, move/transition and transition discovery, bulk properties, bulk email delivery, configurable global Bulk change permission, exact expansions and permissions; see [BULK_ISSUES.md](BULK_ISSUES.md) |
