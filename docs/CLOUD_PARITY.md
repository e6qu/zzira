...
| Automation | Partial | Eight management operations, durable execution, fixed-interval and cron schedules, five work item event triggers, JQL, field and related-work conditions, a related-work branch, smart values, ten work item actions including outbox-queued mail to a work item's assignee, reporter or watchers, a template gallery and audit | Webhook and incoming-request triggers, connections (stored, not executed), web request actions, page creation, work item creation above sub-task level, usage limits and the rest of the action catalog; see [AUTOMATION.md](AUTOMATION.md) |
