...
assesses 471 operations: 464 partial and 7 missing. The remaining 736 operations
...
| Work items | Partial | Issue CRUD, one-level sub-task hierarchy with Jira parent fields, rich-text subset, comments, attachments, worklogs, links, watchers, durable votes with self-service REST/browser journeys, versions, issue-bound Advanced Forms lifecycle, fields, security and single-issue JSON property lifecycle | Complete ADF and metadata; epic and higher-level hierarchy, components, estimates, bulk property operations, exact expansions and permissions |
| Search and filters | Partial | Shared permission-filtered JQL compiler with boolean/numeric/text operators, `NOT IN`, relative and natural-period date functions, immutable `WAS`/`CHANGED` history predicates, relation-backed group/link/sprint/type/version/watch/vote/update/project list functions with current Jira Cloud aliases, label/version/sprint multi-value semantics, deterministic multi-field ordering, immutable numeric Jira issue IDs, durable query/reconciliation/workspace/user-bound seven-day enhanced-search snapshots, strongly consistent reconciliation, legacy/enhanced/count search with field selection, key aliases, all documented expansion names, requested properties, executable transitions and current versioned representations, reference data, permission-filtered suggestions, Jira-shaped parse, bounded numeric-ID bulk match, sanitize and personal-data migration resources, app-only tenant-scoped function-precomputation paging/search/atomic updates, installed Connect/native function declarations, autocomplete, signed cache-miss evaluation, expiring precomputations and safely compiled recursive fragments, navigator, and all 19 pinned filter routes with private/shared visibility, owned/favorite collections, paginated search, user/group/project/role view and edit shares, favorites, columns, owner transfer and default scope | Remaining fields and built-in functions, exact personal-data migration, richer rendering and historical versioned representations, and scheduled subscription delivery; see [JQL.md](JQL.md) and [FILTERS.md](FILTERS.md) |
