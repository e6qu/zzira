...
| Work items | Partial | Issue CRUD, one-level sub-task hierarchy with Jira parent fields, rich-text subset, comments, attachments, worklogs, links, watchers, durable votes with self-service REST/browser journeys, versions, canonical multi-component create/update/search/default-assignment semantics, issue-bound Advanced Forms lifecycle, fields, security and single-issue JSON property lifecycle, plus administrator-gated common-field discovery, durable edits across ten Jira field families, and durable bulk watch/unwatch for up to 1,000 visible items with per-item results and Jira-shaped task progress | Complete ADF and metadata; epic and higher-level hierarchy, estimates, remaining bulk field families, delete/move/transition and transition discovery, bulk properties, bulk email delivery, configurable global Bulk change permission, exact expansions and permissions; see [BULK_ISSUES.md](BULK_ISSUES.md) |
