...
| Automation | Partial | Eight management operations, durable execution, fixed-interval and cron schedules, five work item event triggers, JQL, field and related-work conditions, a related-work branch, smart values, nine idempotent work item actions, a template gallery and audit | Webhook and incoming-request triggers, connections (stored, not executed), email and web request actions, page creation, work item creation above sub-task level, usage limits and the rest of the action catalog; see [AUTOMATION.md](AUTOMATION.md) |
