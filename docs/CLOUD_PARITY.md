...
assesses 203 operations: 196 partial and 7 missing. The remaining 1,004 operations
...
| Service Management | Partial | Service-project creation, atomic desk/type/queue provisioning, customer help center and request journey, customer-only identity activation/revocation, open/closed portal membership, organizations/members/properties/desk links, participants, public/internal conversation and attachments, approvals, subscriptions, CSAT, status transitions, incident-to-DORA linkage, per-desk agent roles, scoped all-request access, business calendars, first-response/resolution cycles, SLA attention and durable escalation notifications, agent queue/assignment and customer-directory workspace, and 64 pinned operations | Custom queues/JQL, dynamic forms, advanced SLA goal criteria, holidays, approval configuration, email delivery, CSAT reports, knowledge-base suggestions, Assets, incident/problem/change configuration, service reports and the remaining 11 REST operations; see [SERVICE_MANAGEMENT.md](SERVICE_MANAGEMENT.md) |
