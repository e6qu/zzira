...
| App runtime | Partial | Administrator-authorized native and standard Connect descriptor install/reinstall with encrypted shared secrets, translated scopes, HMAC-signed fresh and replay-protected lifecycle/storage/product-API requests, Connect-compatible HS256 JWT/QSH product authentication and callbacks, stable scope-enforced app principals across Jira/Agile/JSM/Confluence REST, isolated versioned JSON storage, scope-safe upgrades, suspension/uninstall, organization audit, host-rendered or sandboxed signed remote Jira/Confluence global pages, static and dynamically registered issue panels, dashboard gadgets and content byline items, Jira/Confluence-path dynamic-module register/list/remove for remote panels and keyed JQL-filtered webhooks with restoration and upgrade-conflict semantics, descriptor-declared outbound lifecycle delivery, four scheduled-trigger intervals, durable bounded retry/recovery and recent-delivery administration; core webhooks and entity properties | Remaining Connect module families, dynamic module types and webhook options, Forge-hosted compute, workflow/custom-field modules and upgrade migrations; see [APPS.md](APPS.md) |
