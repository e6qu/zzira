...
assesses 214 operations: 207 partial and 7 missing. The remaining 993 operations
...
| Reports and metrics | Partial | Dashboard statistics/pie tables, release progress, immutable delivery facts and permission-filtered 7/30/90 day DORA summary with accessible daily chart/table and recent production evidence | Jira and Agile report catalog, configurable DORA mappings and targets, service metrics, comparisons, exports and scheduled delivery; see [REPORTS.md](REPORTS.md) |
...
| Service Management | Partial | Service-project creation, atomic desk/type/queue provisioning, customer help center and request journey, customer-only identity activation/revocation, open/closed portal membership, organizations/members/properties/desk links, participants, public/internal conversation and attachments, approvals, subscriptions, CSAT, status transitions, incident-to-DORA linkage, per-desk agent roles, scoped all-request access, built-in and manager-defined JQL queues, business calendars, first-response/resolution cycles, SLA attention and durable escalation notifications, agent queue/assignment and customer-directory workspace, linked Confluence knowledge suggestions, request-type groups/permissions/properties, Assets workspace discovery, and all 75 pinned operations reviewed | Complete JQL, dynamic forms, advanced SLA goal criteria, holidays, approval configuration, email delivery, CSAT reports, complete Assets object/schema/import APIs, incident/problem/change configuration and service reports; see [SERVICE_MANAGEMENT.md](SERVICE_MANAGEMENT.md) |
