...
assesses 120 operations: 113 partial and 7 missing. The remaining 1,087 operations
...
| Workflows | Partial | Workspace-scoped custom status lifecycle, all thirteen Jira status/status-category/usage operations as reviewed subsets, workspace-owned workflows, versioned workflow and scheme drafts, Jira published/draft/bulk scheme resources, modern paged/filterable workflow search with status and transition expansion, selector-aware workflow capability discovery, structured non-mutating create/update validation for existing statuses and directed topology, paged workflow project/scheme/issue-type usage and scheme project usage, guarded inactive workflow deletion, new-editor discovery, required-mapping analysis, optimistic document versions, runtime scheme resolution, project impact preview, atomic status replacement during switch/publish/update, all thirty-five pinned workflow-scheme REST operations as reviewed subsets, durable completed-task polling and audit | Project status scopes, atomic bulk status writes, queued/running tasks, workflow create/update mutations, new-status validation, configurable capability catalogs, designer persistence, conditions, validators and post-functions |
