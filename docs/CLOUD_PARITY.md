...
| Workflows | Partial | Global and project-scoped custom status lifecycle with isolated names, Jira scope beans and project-aware search, all thirteen Jira status/status-category/usage operations as reviewed subsets, workspace-owned workflows, versioned workflow and scheme drafts, Jira published/draft/bulk scheme resources, modern paged/filterable workflow search with status and transition expansion, selector-aware workflow capability discovery with executable system-rule entries, project-associated workflow preview by ID/name/issue type with query context, structured create/update validation plus atomic batch creation of global statuses and workflows and atomic workflow updates, modern API persistence for descriptions, start/loop coordinates, status properties and layouts, a connected drag/keyboard admin designer with serialized draft saves, nested ALL/ANY reporter/assignee/account transition restrictions, required-field validators, atomic current/selected/unassigned assignee post-functions, rule round-tripping across search and preview, scoped-over-default active work-item status migration with sync actions, directed topology, optimistic workflow versions, active issue-status safety, whole-batch rollback and audit, paged workflow project/scheme/issue-type usage and scheme project usage, guarded inactive workflow deletion, new-editor discovery, required-mapping analysis, runtime scheme resolution, project impact preview, atomic status replacement during scheme switch/publish/update, all thirty-five pinned workflow-scheme REST operations as reviewed subsets, durable ENQUEUED/RUNNING/terminal task execution, creator/admin polling, cancellation rollback, failure results, stale-claim recovery and audit | Remaining system and ecosystem rules, atomic bulk status writes, project-scoped workflows and capability catalogs, and transition screens |
