...
var moduleRequirements = map[string]struct {
	...
	"jira:adminPage":               {Location: "jira.admin", Scope: "read:jira-work"},
	"jira:configurePage":           {Location: "jira.app.configure", Scope: "read:jira-work"},
	"jira:profilePage":             {Location: "jira.profile", Scope: "read:jira-work"},
...
func validateDescriptorWire(wire descriptorWire) (models.AppDescriptor, error) {
	...
	moduleTitleLimit := 255
	if wire.Format == "connect" {
		moduleTitleLimit = 1500
	}
...
func validAppCallbackPath(value string) bool {
	if len(value) == 0 || len(value) > 2048 || value[0] != '/' ||
		(len(value) > 1 && (value[1] == '/' || value[1] == '\\')) || strings.ContainsAny(value, "\\\r\n") {
		return false
	}
