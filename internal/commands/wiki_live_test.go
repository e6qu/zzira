...
func TestWikiLiveDocumentsMergeEditorsInRevisionOrder(t *testing.T) {
	...
	stale, err := st.ApplyWikiLiveChanges(ctx, ws, ben, page.ID, session, 0, []store.WikiLiveChange{{Position: 10, Delete: 0, Insert: " today"}})
	...
	if _, err := st.ApplyWikiLiveChanges(ctx, ws, ben, page.ID, session, 1, []store.WikiLiveChange{{Position: 13, Delete: 0, Insert: " today"}}); err != nil {
		t.Fatal(err)
	}
	caughtUp, err := st.WikiLiveDocument(ctx, ws, ana, page.ID, session, 1)
	...
	if _, err := st.ApplyWikiLiveChanges(ctx, ws, ana, page.ID, session, 2, []store.WikiLiveChange{{Position: 3, Insert: "🚀"}}); err != nil {
		t.Fatal(err)
	}
	if _, err := st.ApplyWikiLiveChanges(ctx, ws, ana, page.ID, session, 3, []store.WikiLiveChange{{Position: 4, Delete: 1}}); !errors.Is(err, store.ErrWikiValidation) {
		t.Fatalf("split character error = %v", err)
	}
	if _, err := st.ApplyWikiLiveChanges(ctx, ws, ana, page.ID, session, 3, []store.WikiLiveChange{{Position: 999, Delete: 1}}); !errors.Is(err, store.ErrWikiValidation) {
		t.Fatalf("out of range error = %v", err)
	}
	// Text that is not yet valid storage is shared but not drafted.
	if _, err := st.ApplyWikiLiveChanges(ctx, ws, ana, page.ID, session, 3, []store.WikiLiveChange{{Position: 0, Insert: "<stro"}}); err != nil {
		t.Fatal(err)
	}
	if draft, err = st.WikiContentDraft(ctx, ws, ana, "page", page.ID); err != nil || draft.Body.Value != "<p>🚀We Ship it today</p>" {
	...
	restarted, err := st.WikiLiveDocument(ctx, ws, ana, page.ID, session, 4)
	...
	if _, err := st.ApplyWikiLiveChanges(ctx, ws, ana, page.ID, session, 4, []store.WikiLiveChange{{Position: 0, Insert: "x"}}); !errors.Is(err, store.ErrWikiLiveStale) {
		t.Fatalf("old session error = %v", err)
	}

	// Closing ends the session, and the next editor starts a new one.
	if err := st.CloseWikiLiveDocument(ctx, ws, page.ID); err != nil {
		t.Fatal(err)
	}
	reopened, err := st.WikiLiveDocument(ctx, ws, ben, page.ID, restarted.Session, 0)
