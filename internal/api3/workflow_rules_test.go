...
func TestWorkflowRulesPersistAndExecuteAcrossAPIJourney(t *testing.T) {
	...
	workflowForm, err := st.AttachIssueForm(ctx, workspaceID, issue.ID, "release-checklist", "Release checklist")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := st.SetIssueFormStatus(ctx, workspaceID, issue.ID, workflowForm.ID, true); err != nil {
		t.Fatal(err)
	}
