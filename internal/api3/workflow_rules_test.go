...
func TestWorkflowRulesPersistAndExecuteAcrossAPIJourney(t *testing.T) {
	...
	for _, ruleKey := range []string{"system:restrict-issue-transition", "system:restrict-from-all-users", "system:check-field-value", "system:previous-status-condition", "system:validate-field-value", "system:previous-status-validator", "system:change-assignee", "system:update-field", "system:copy-value-from-other-field", "system:transition-screen"} {
	...
	issue, _, err = st.UpdateIssue(ctx, reporterID, workspaceID, issue.ID, store.IssueUpdate{StatusID: &inProgress, ExpectedUpdatedSeq: &issue.UpdatedSeq})
	if err != nil {
		t.Fatal(err)
	}
	toDo := "st_todo"
	issue, _, err = st.UpdateIssue(ctx, reporterID, workspaceID, issue.ID, store.IssueUpdate{StatusID: &toDo, ExpectedUpdatedSeq: &issue.UpdatedSeq})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, fragment := range []string{`"description":"A governed release path"`, `"layout":{"x":40,"y":60}`, `"startPointLayout":{"x":-100,"y":-80}`, `"conditions":{"operation":"ALL"`, `"ruleKey":"system:restrict-from-all-users"`, `"ruleKey":"system:check-field-value"`, `"ruleKey":"system:previous-status-condition"`, `"ruleKey":"system:validate-field-value"`, `"ruleKey":"system:previous-status-validator"`, `"ruleKey":"system:change-assignee"`, `"ruleKey":"system:update-field"`, `"ruleKey":"system:copy-value-from-other-field"`} {
