...
func TestFilterAdministrationContractJourney(t *testing.T) {
	...
	subscription, err := st.SaveFilterSubscription(ctx, workspaceID, ownerID, st.FilterIDByRef(ctx, workspaceID, filterID), store.FilterSubscriptionInput{Expression: "0 8 * * *", Recipients: []string{ownerID, memberID}})
	if err != nil {
		t.Fatal(err)
	}
