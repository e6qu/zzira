...
func TestAttachedFieldsCompile(t *testing.T) {
	for _, tc := range []struct {
		...
		{query: `Organizations = "Riverbank"`, contains: []string{"service_request_organizations request_organization", "lower(organization.name)="}, args: []any{"Riverbank"}},
		{query: `organization IN ("Riverbank", "Kestrel")`, contains: []string{"service_request_organizations request_organization"}, args: []any{"Riverbank", "Kestrel"}},
		{query: `Organizations IS EMPTY`, contains: []string{"(NOT EXISTS (SELECT 1 FROM service_request_organizations request_organization"}},
