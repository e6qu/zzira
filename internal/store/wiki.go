...
const wikiPageSelect = `SELECT p.id::text,s.workspace_id,p.space_id::text,COALESCE(COALESCE(p.parent_id,p.parent_content_id)::text,''),CASE WHEN p.parent_content_id IS NOT NULL THEN (SELECT pc.type FROM wiki_content pc WHERE pc.id=p.parent_content_id) WHEN p.parent_id IS NOT NULL THEN 'page' ELSE '' END,p.title,p.status,p.published,p.classification_level,p.body,p.author_id,to_char(p.created_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"'),v.version,v.message,v.minor_edit,v.author_id,to_char(v.created_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"'),p.position FROM wiki_pages p JOIN wiki_spaces s ON s.id=p.space_id JOIN wiki_page_versions v ON v.page_id=p.id AND v.version=p.version`
...
func scanWikiSpace(row pgx.Row) (*models.WikiSpace, error) {
	s := &models.WikiSpace{}
	err := row.Scan(&s.ID, &s.WorkspaceID, &s.Key, &s.Name, &s.Description, &s.AuthorID, &s.Private,
		&s.DefaultClassificationLevel, &s.CreatedAt, &s.Type, &s.Alias, &s.HomepageID, &s.Status,
		&s.RouteOverrideEnabled, &s.ContentMode, &s.ThemeKey)
	return s, err
}
func scanWikiPage(row pgx.Row) (*models.WikiPage, error) {
	p := &models.WikiPage{Body: models.WikiBody{Representation: "storage"}}
	err := row.Scan(&p.ID, &p.WorkspaceID, &p.SpaceID, &p.ParentID, &p.ParentType, &p.Title, &p.Status, &p.Published, &p.ClassificationLevel, &p.Body.Value, &p.AuthorID, &p.CreatedAt, &p.Version.Number, &p.Version.Message, &p.Version.MinorEdit, &p.Version.AuthorID, &p.Version.CreatedAt, &p.Position)
	return p, err
}
...
func (s *Store) SaveWikiPage(ctx context.Context, ws, actor string, input models.WikiPage) (*models.WikiPage, error) {
	...
	if input.ID == "" {
		err = tx.QueryRow(ctx, `INSERT INTO wiki_pages(space_id,parent_id,parent_content_id,title,status,body,author_id,published,classification_level,position)
			VALUES ($1::bigint,CASE WHEN $8::text='page' THEN $2::bigint END,CASE WHEN $8::text NOT IN ('page','') THEN $2::bigint END,$3,$4,$5,$6,$4='current',$7,$9) RETURNING id::text`,
			spaceID, nilIfEmpty(input.ParentID), input.Title, input.Status, input.Body.Value, actor, input.ClassificationLevel, parentType, position).Scan(&input.ID)
	} else {
		_, err = tx.Exec(ctx, `UPDATE wiki_pages SET parent_id=CASE WHEN $7::text='page' THEN $2::bigint END,
			...
			input.ID, nilIfEmpty(input.ParentID), input.Title, input.Status, input.Body.Value, input.Version.Number, parentType, reparented, position)
	}
	if err != nil {
		return nil, err
	}
