...
// SearchSnapshotOptions are the choices a materialized search remembers, so
// its later pages answer the question its first page was asked.
type SearchSnapshotOptions struct {
	IncludeArchivedProjects bool
}

func (s *Store) CreateSearchSnapshot(ctx context.Context, workspaceID, userID, queryHash string, compiled jql.Compiled, expiresAt time.Time, options SearchSnapshotOptions) (string, error) {
	if compiled.Err != nil {
		return "", compiled.Err
	}
	...
	if _, err = tx.Exec(ctx, `
		INSERT INTO jira_search_snapshots(id, workspace_id, user_id, query_hash, expires_at, include_archived_projects)
		VALUES($1,$2,NULLIF($3,''),$4,$5,$6)`, snapshotID, workspaceID, userID, queryHash, expiresAt, options.IncludeArchivedProjects); err != nil {
		return "", err
	}
	...
	where += " AND " + VisibleIssuePredicateIn("i", userPlaceholder, options.IncludeArchivedProjects)
...
func (s *Store) SearchSnapshotPage(ctx context.Context, snapshotID, workspaceID, userID, queryHash string, position int64, limit int) (*SearchSnapshotPage, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	var includeArchivedProjects bool
	err = tx.QueryRow(ctx, `
		SELECT include_archived_projects FROM jira_search_snapshots
		WHERE id=$1 AND workspace_id=$2 AND user_id IS NOT DISTINCT FROM NULLIF($3,'') AND query_hash=$4 AND expires_at > now()`,
		snapshotID, workspaceID, userID, queryHash).Scan(&includeArchivedProjects)
	...
	rows, err := tx.Query(ctx, `
		...
		WHERE item.snapshot_id=$1 AND item.position >= $2
		  AND i.workspace_id=$3 AND `+VisibleIssuePredicateIn("i", "$4", includeArchivedProjects)+`
	...
	issueRows, err := tx.Query(ctx, issueJoin+`
		WHERE i.workspace_id=$1 AND i.id=ANY($2::TEXT[]) AND `+VisibleIssuePredicateIn("i", "$3", includeArchivedProjects), workspaceID, ids, userID)
	if err != nil {
		return nil, err
	}
