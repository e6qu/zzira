...
func (s *Store) WikiAttachment(ctx context.Context, ws, user, id string) (*models.WikiAttachment, error) {
	return scanWikiAttachment(s.Pool.QueryRow(ctx, wikiAttachmentSelect+` WHERE s.workspace_id=$1 AND `+wikiSpaceVisible+` AND `+wikiPageVisible+` AND a.id::text=$3`, ws, user, id))
}
...
func (s *Store) WikiAttachments(ctx context.Context, ws, user, pageID, mediaType, filename, status string) ([]*models.WikiAttachment, error) {
	rows, err := s.Pool.Query(ctx, wikiAttachmentSelect+` WHERE s.workspace_id=$1 AND `+wikiSpaceVisible+` AND `+wikiPageVisible+` AND ($3='' OR a.page_id::text=$3) AND ($4='' OR a.media_type=$4) AND ($5='' OR a.filename=$5) AND ($6='' OR a.status=$6) ORDER BY a.id`, ws, user, pageID, mediaType, filename, status)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SaveWikiAttachment(ctx context.Context, ws, actor, pageID, attachmentID, filename, mediaType, comment, message string, minor bool, size int64, blobRef string) (*models.WikiAttachment, error) {
	...
	if err = wikiAction(ctx, tx, ws, actor, "wiki_attachment", a.ID, spaceID, a); err != nil {
		return nil, err
	}
...
func (s *Store) DeleteWikiAttachment(ctx context.Context, ws, actor, id string) ([]string, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	a, err := scanWikiAttachment(tx.QueryRow(ctx, wikiAttachmentSelect+` WHERE s.workspace_id=$1 AND `+wikiSpaceVisible+` AND `+wikiPageVisible+` AND `+wikiPageWritable+` AND a.id::text=$3 FOR UPDATE OF a`, ws, actor, id))
	if err != nil {
		return nil, err
	}
	...
	rows.Close()
	var spaceID string
	if err = tx.QueryRow(ctx, `SELECT space_id::text FROM wiki_pages WHERE id::text=$1`, a.PageID).Scan(&spaceID); err != nil {
		return nil, err
	}
	payload, err := json.Marshal(map[string]any{"wikiSpaceId": spaceID, "wiki_attachment": a})
	if err != nil {
		return nil, err
	}
	...
	if err = appendAction(ctx, tx, &models.Action{WorkspaceID: ws, Seq: seq, EntityType: "wiki_attachment", EntityID: id, Op: models.OpDelete, SchemaV: models.SchemaVersion, Payload: payload, ActorID: actor}); err != nil {
		return nil, err
	}
	if err = tx.Commit(ctx); err != nil {
		return nil, err
