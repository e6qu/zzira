...
type PlanView struct {
	...
	GroupBy string
	Query   string
	// RollUp is whether the work under a parent is added up onto it.
	RollUp bool
}
...
func (s *Store) PlanViews(ctx context.Context, workspaceID string, planID int64) ([]PlanView, error) {
	rows, err := s.Pool.Query(ctx, `SELECT id,name,group_by,query,rollup FROM plan_views
	...
	for rows.Next() {
		var view PlanView
		if err := rows.Scan(&view.ID, &view.Name, &view.GroupBy, &view.Query, &view.RollUp); err != nil {
			return nil, err
		}
...
func (s *Store) PlanViewByID(ctx context.Context, workspaceID string, planID, viewID int64) (*PlanView, error) {
	view := &PlanView{}
	err := s.Pool.QueryRow(ctx, `SELECT id,name,group_by,query,rollup FROM plan_views
		WHERE workspace_id=$1 AND plan_id=$2 AND id=$3`, workspaceID, planID, viewID).Scan(&view.ID, &view.Name, &view.GroupBy, &view.Query, &view.RollUp)
...
func (s *Store) SavePlanView(ctx context.Context, workspaceID, actorID string, planID int64, view PlanView) (*PlanView, error) {
	...
	err := s.Pool.QueryRow(ctx, `INSERT INTO plan_views(workspace_id,plan_id,name,group_by,query,rollup,created_by)
		VALUES($1,$2,$3,$4,$5,$6,$7)
		ON CONFLICT(workspace_id,plan_id,lower(name)) DO UPDATE SET group_by=EXCLUDED.group_by,query=EXCLUDED.query,rollup=EXCLUDED.rollup
		RETURNING id`, workspaceID, planID, view.Name, view.GroupBy, view.Query, view.RollUp, actorID).Scan(&view.ID)
	return &view, err
}
