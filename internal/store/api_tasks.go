...
func (r *APITaskRunner) execute(ctx context.Context, task APITask) error {
	...
	case apiTaskDeletePriority:
		return r.Store.executePriorityDeletion(ctx, task)
	case apiTaskDeleteResolution:
	...
	case apiTaskCreateProjectFromTemplate:
		return r.Store.executeProjectFromTemplate(ctx, task)
