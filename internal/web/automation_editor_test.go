...
func TestAutomationEditorKeepsWhatItCannotShow(t *testing.T) {
	for payload, want := range map[string]string{
		...
		`{"trigger":{"type":"jira.issue.event.trigger:created"},"components":[{"component":"BRANCH","type":"jira.issue.related","children":[{"component":"CONDITION","type":"jira.jql.condition"}]}]}`:                                                                                                         "its branches",
