-- An archived project is out of sight, and its work with it, because this
-- function answers no for a project that is not active. Jira's enhanced
-- search can be asked to include that work, so the question grows a way to
-- say so; everything else asks it as before and gets the same answer,
-- because the new argument defaults to false.
--
-- The whole body is restated, as every change to this function has been:
-- the newest migration is where it is read.
