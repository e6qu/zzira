...
| Workflows | Workspace status lifecycle with impact-safe deletion, Jira status REST subset, workspace-owned workflow directory, versioned workflow and scheme drafts, issue-type scheme mappings, impact preview, atomic status replacement during project switches, durable completed-task polling, explicit publish/discard with audit, runtime isolation from drafts and other sites, and transition enforcement | Remaining workflow-scheme APIs, queued task execution, project status scopes, conditions, validators, post-functions and visual designer persistence |
