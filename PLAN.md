...
| Workflows | Workspace status lifecycle with impact-safe deletion, Jira status REST subset, workspace-owned workflow directory, versioned workflow and scheme drafts, Jira published/draft default/issue-type/workflow mappings and project usage, impact preview, atomic status replacement during project switches, durable completed-task polling, explicit publish/discard with audit, runtime isolation from drafts and other sites, and transition enforcement | Bulk workflow-scheme APIs, publish-time status migration, queued task execution, project status scopes, conditions, validators, post-functions and visual designer persistence |
