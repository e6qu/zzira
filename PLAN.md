...
| Apps | Administrator-authorized native and standard Connect descriptor installation/reinstallation with encrypted secrets, explicit translated scopes, signed and replay-protected inbound lifecycle callbacks, Connect-compatible HS256 JWT/QSH product API authentication, isolated versioned JSON storage, stable scope-enforced app principals across Jira/Agile/JSM/Confluence REST, scope-safe upgrades, suspension/uninstall, audited administration, host-rendered or signed remote Jira/Confluence global pages, issue panels, dashboard gadgets and content byline items, descriptor-declared outbound lifecycle callbacks with Connect JWT, JQL-filtered webhooks, four scheduled-trigger intervals, durable retry/recovery and delivery administration; core webhooks and entity properties | Remaining Connect module families, dynamic modules, Forge-hosted compute, workflow/custom-field modules and upgrade migrations |
...
14. **App runtime.** Extend the delivered standard Connect descriptor and
    signed remote iframe runtime with remaining module families, dynamic
    modules, issue tabs, workflow modules, custom fields and upgrade migrations
