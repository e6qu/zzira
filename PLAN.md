...
| Workflows | Workspace status lifecycle with impact-safe deletion, Jira status REST subset, workspace-owned workflow directory, versioned workflow and scheme drafts, Jira published/draft/bulk scheme resources, paged workflow and scheme usage resources, required-mapping analysis, optimistic document versions, impact preview, atomic status replacement during project switch, draft publish, and bulk update, durable completed-task polling, explicit publish/discard with audit, runtime isolation from drafts and other sites, and transition enforcement | Queued task execution, project status scopes, conditions, validators, post-functions and visual designer persistence |
