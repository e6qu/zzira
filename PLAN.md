...
| Workflows | Workspace status lifecycle with impact-safe deletion, Jira status REST subset, workspace-owned workflow directory, versioned workflow and scheme drafts, Jira published/draft/bulk scheme resources, modern paged/filterable workflow search with status and transition expansion, selector-aware workflow capability discovery, project-associated workflow preview by ID/name/issue type, structured validation and atomic batch creation of global statuses and workflows plus atomic workflow updates, modern API description/start/loop/status layout persistence, a connected drag/keyboard designer with serialized draft saves, nested actor conditions, required-field validators and atomic assignee post-functions with modern API round-tripping, scoped-over-default active work-item status migration with sync actions, directed topology, optimistic workflow versions, active issue-status safety, whole-batch rollback and audit, paged workflow and scheme usage resources, guarded inactive workflow deletion, new-editor discovery, required-mapping analysis, optimistic scheme document versions, impact preview, atomic status replacement during project switch, draft publish, and bulk update, durable queued task execution, polling, cancellation, failure results, and stale-claim recovery, explicit publish/discard with audit, runtime isolation from drafts and other sites, and transition enforcement | Project status scopes, remaining system/ecosystem rules, project-scoped capability catalogs and transition screens |
