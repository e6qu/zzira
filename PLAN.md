...
| Workflows | Workspace status lifecycle with impact-safe deletion, Jira status REST subset, workspace-owned workflow directory, versioned draft editing, explicit publish/discard with audit, project assignment, runtime isolation from drafts and other sites, and transition enforcement | Workflow schemes, conditions, validators, post-functions and visual designer persistence |
