...
|---|---|---|
| Identity | Password, API tokens, sessions, one configured OIDC provider, back-channel logout | Provider registry, Atlassian/Google/Microsoft login, account linking, organizations and product access |
...
| Administration | Organization/site/product/directory foundation, DNS domain claims, audited group and managed-account lifecycle, profiles, directory-scoped suspension, role access, search, statistics and event APIs; limited project settings | Remaining organization identity/policy APIs and Jira/Confluence schemes |
