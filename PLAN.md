...
| Workflows | Global and project-scoped status and workflow lifecycles with impact-safe deletion, isolated ownership and names, Jira scope resources, project-aware search and capability catalogs, versioned workflow and scheme drafts, modern status and transition expansion, project-associated preview, structured validation and atomic workflow/status batches, modern designer metadata, a connected drag/keyboard designer with serialized draft saves and transition-screen field selection, nested actor conditions, required-field validators, atomic assignee post-functions, screen-authorized atomic field/status changes and API round-tripping, active work-item status migration with sync actions, directed topology, optimistic versions, active issue-status safety, whole-batch rollback and audit, paged usage resources, guarded inactive deletion, required-mapping analysis, impact preview, atomic status replacement, durable queued task execution/cancellation/recovery, explicit publish/discard, runtime isolation, and transition enforcement | Remaining system/ecosystem rules, atomic bulk status writes, complete visual rule editing and exact team-managed workflow routing |
