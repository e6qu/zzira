...
| Workflows | Global and project-scoped status and workflow lifecycles with impact-safe deletion, isolated ownership and names, Jira scope resources, project-aware search and capability catalogs, atomic multi-status create/update/delete with simultaneous renames, versioned drafts, modern expansion, project-associated preview, structured validation and atomic workflow/status batches, designer metadata, a connected drag/keyboard designer with serialized saves and visual actor, API-only, field-value, required-field, assignee, field-update and transition-screen controls, nested actor, request-source, and typed field comparison conditions, validators, atomic assignee and system/custom-field post-functions, screen-authorized field/status changes and API round-tripping, active work-item migration with sync actions, directed topology, optimistic versions, active status safety, rollback and audit, paged usage, guarded deletion, impact preview, atomic status replacement, durable task execution/cancellation/recovery, explicit publish/discard, runtime isolation, and transition enforcement | Remaining system/ecosystem rule types and advanced parameters and exact team-managed workflow routing |
