...
test('wiki space, rich page, access, stale edits, child pages, history, trash and restore', async ({ page, context, browser }) => {
  ...
  await page.getByLabel('Title', { exact: true }).fill('Engineering release update');
  await page.getByLabel('Content', { exact: true }).fill('The release candidate is ready for staged rollout.');
  ...
  await page.getByRole('button', { name: 'Save blog post', exact: true }).click();
  await expect(page.getByRole('article', { name: 'Blog post content' })).toContainText('reached production');
  ...
  await page.getByLabel('Add label').fill('release-update');
  await page.getByRole('button', { name: 'Add label', exact: true }).click();
  await expect(page.getByRole('region', { name: 'Blog post details' })).toContainText('global:release-update');
  await page.getByText('App properties · 0', { exact: true }).click();
  await page.getByLabel('New property key').fill('release-metadata');
  ...
  await expect(page.getByRole('region', { name: 'Blog posts' })).toContainText('Engineering release update');
  await page.getByRole('link', { name: 'Engineering release update', exact: true }).click();
  await page.getByText('Restore or purge', { exact: true }).click();
  ...
  await databases.getByLabel('Database name').fill('Service catalog');
  ...
  await serviceCatalog.getByText('Set classification', { exact: true }).click();
  await serviceCatalog.getByLabel('Classification level').selectOption('confidential');
  await serviceCatalog.getByRole('button', { name: 'Save classification', exact: true }).click();
  await expect(page.locator('.wiki-databases li').filter({ hasText: 'Service catalog' })).toContainText('Classification: Confidential');
  ...
  await whiteboards.getByLabel('Whiteboard name').fill('Incident review canvas');
  await whiteboards.getByLabel('Template', { exact: true }).selectOption('incident-postmortem');
  ...
  await incidentCanvas.getByText('Set classification', { exact: true }).click();
  await incidentCanvas.getByLabel('Classification level').selectOption('restricted');
  await incidentCanvas.getByRole('button', { name: 'Save classification', exact: true }).click();
  await expect(page.locator('.wiki-whiteboards li').filter({ hasText: 'Incident review canvas' })).toContainText('Classification: Restricted');
  await checkWikiAccessibility(page);
