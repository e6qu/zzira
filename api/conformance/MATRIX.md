...
| Service management surfaces | 🟡 | 64 pinned operations cover service projects, request types, requests, queues, SLAs, approvals, attachments, subscriptions, feedback, customer-only account lifecycle, desk customer admission, and customer organizations/members/properties/desk links. Help-center, customer, agent, and manager journeys use the same command paths; dynamic forms, custom queues, knowledge suggestions, Assets and 11 operations remain |
