...
| Service management surfaces | 🟡 | All 75 pinned operations are reviewed: service projects, request types/groups/permissions/properties, requests, durable request-type field metadata and typed custom answers, built-in and manager-defined JQL queues, SLAs and holiday calendars, approvals, attachments, subscriptions, feedback, customer-only account lifecycle, desk customer admission, customer organizations, linked knowledge search, Assets workspace discovery, and initial service volume/SLA/CSAT reports. Help-center, customer, agent, and manager journeys use the same command paths; conditional/advanced portal fields, complete JQL, complete Assets APIs, incident/problem/change configuration and advanced report segmentation remain |
