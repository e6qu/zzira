...
| GET /rest/api/3/search · POST /search · GET/POST /search/jql · POST /search/approximate-count | 🟡 | Permission-filtered JQL with history/date operators; legacy/enhanced search support field selection/exclusion, app-key aliases, names/schema/rendered expansion, requested properties, strict bodies, bounded count queries, IDs-only enhanced defaults, deterministic ordering and query/user/workspace-bound seven-day tokens; remaining issue expansions, reconciliation and snapshot/keyset semantics remain |
