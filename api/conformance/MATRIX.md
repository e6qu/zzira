...
| Service management surfaces | 🟡 | All 75 pinned operations are reviewed: service projects, request types/groups/permissions/properties, requests, queues, SLAs, approvals, attachments, subscriptions, feedback, customer-only account lifecycle, desk customer admission, customer organizations, linked knowledge search and Assets workspace discovery. Help-center, customer, agent, and manager journeys use the same command paths; dynamic forms, custom queues, complete Assets APIs, incident/problem/change configuration and service reports remain |
