...
| GET /rest/api/3/search · POST /search · GET/POST /search/jql · POST /search/approximate-count | 🟡 | Permission-filtered JQL with history/date operators; legacy/enhanced search support field selection/exclusion, app-key aliases, documented expansion names, requested properties, executable transitions, immutable changelogs, current versioned representations, strict bodies, bounded count queries, IDs-only enhanced defaults, deterministic ordering and query/user/workspace-bound seven-day tokens; richer rendering/history, reconciliation and snapshot/keyset semantics remain |
