...
func main() {
	...
	mux.HandleFunc("GET /saml/metadata", webHandler.SAMLMetadata)
	mux.HandleFunc("GET /saml/{provider}/metadata", webHandler.SAMLMetadata)
	mux.HandleFunc("GET /saml/{provider}/login", webHandler.SAMLLogin)
	mux.HandleFunc("POST /saml/{provider}/acs", webHandler.SAMLAssertion)
	...
	mux.HandleFunc("POST /service/requests/{key}/share", webHandler.ServiceRequestShare)
	...
	mux.HandleFunc("POST /service/agent/{desk}/assets/files", webHandler.ServiceAssetObjectFile)
	mux.HandleFunc("GET /service/agent/{desk}/assets/objects/{object}/files/{file}", webHandler.ServiceAssetObjectFileDownload)
	...
	mux.HandleFunc("POST /admin/saml-providers", webHandler.SAMLProviderSettings)
	...
	mux.HandleFunc("POST /wiki/spaces/import", webHandler.WikiSpaceImport)
	...
	mux.HandleFunc("POST /plans/{id}/auto-schedule", webHandler.PlanAutoSchedule)
	mux.HandleFunc("POST /plans/{id}/views", webHandler.PlanViewSave)
	...
	mux.HandleFunc("POST /profile/language", webHandler.UpdateLanguage)
	...
	mux.HandleFunc("POST /profile/passkeys/options", webHandler.PasskeyRegistrationOptions)
	mux.HandleFunc("POST /profile/passkeys", webHandler.PasskeyRegister)
	mux.HandleFunc("POST /profile/passkeys/{credential}/delete", webHandler.PasskeyDelete)
	mux.HandleFunc("POST /login/verify/passkey/options", webHandler.PasskeySignInOptions)
	mux.HandleFunc("POST /login/verify/passkey", webHandler.PasskeySignIn)
